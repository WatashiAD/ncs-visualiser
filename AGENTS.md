...
- **Fluid continuous word fill & anti-jitter glyphs**:
  ...
  - *Fix*: Replaced per-letter text clipping with GPU-composited `color-mix(in srgb, var(--vis-sung-color) ${sungPct}%, var(--vis-unsung-color))` and smooth opacity ramp (`0.40` to `0.95`). Upcoming letters receive gentle pre-illumination via Gaussian bell curve weighting, creating fluid left-to-right vocal color fill across the word with zero font rasterization lag.